...
- No error.
- `Rule` is `.local/share/containers`.
- `Flags` is `cache`.
- `Owner` is empty.
- `Reason` non-empty.
...
- Matching whole `.local` or wrong flags fails.
...
func Assert(t *testing.T, req *Request, resp *Response, err error) {
	...
	if resp.Rule != ".local/share/containers" {
		t.Fatalf("Rule: got %q, want .local/share/containers", resp.Rule)
	}
	if resp.Flags != "cache" {
